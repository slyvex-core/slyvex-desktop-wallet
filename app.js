...
class App extends FlowApp{
	...
	async initConfig(){
		...
		if(reset) {
			try {
				...
				alert('Error resetting Slyvex Desktop data folder: '+ex);
			}
	...
	*/
	dataDirInitError(){
		console.log(`Please start app with --init=/path/to/data/dir or --init for default (~/.slyvex-desktop/data)`);
		this.exit();
	}
	...
	*/
	...
	async removeDataDir(){
		try {
			let datadir2path = path.join(this.dataFolder, "slyvexd-kd0", "slyvex-mainnet", "datadir2");
	...
	*/
	...
	setSkipUTXOIndex(skipUTXOIndex){
		...
		return this.setModuleArgs("slyvexd:", {}, {"skip-utxoindex":skipUTXOIndex})
	}
	getSkipUTXOIndex(){
		let {args, params} = this.getModuleArgs("slyvexd:");
	...
	setUpnpIfMissing(upnpEnabled) {
		if (this.config?.modules) {
			Object.keys(this.config.modules).forEach((k) =>{
				if (k.startsWith('slyvexd:') && !('upnpEnabled' in this.config.modules[k])) {
