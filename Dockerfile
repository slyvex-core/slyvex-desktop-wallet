...
RUN addgroup -S slyvex-desktop && adduser -S slyvex-desktop -G slyvex-desktop
RUN mkdir -p /run/postgresql
RUN chown slyvex-desktop:slyvex-desktop /run/postgresql

# Tell docker that all future commands should run as the appuser user

WORKDIR /home/slyvex-desktop/releases/slyvex-desktop
COPY . .
RUN rm -rf node_modules
RUN rm package-lock.json
#RUN chown -R slyvex-desktop:slyvex-desktop .


RUN npm install
RUN npm install @slyvex/process-list
...
ENTRYPOINT ["node","slyvex-desktop.js","--init","--nice","--mine"]
