...
const {RPC} = require('@slyvex/grpc-node');
...
//TODO
window.PWA_MODULES={};
window.PWA_MODULES["@slyvex/wallet-pwa"] = "N/A";
window.PWA_MODULES["@aspectron/flow-ux"] = "N/A";
window.PWA_MODULES["@slyvex/ux"] = "N/A";
window.PWA_MODULES["@slyvex/grpc-web"] = "N/A";
window.PWA_MODULES["@slyvex/wallet"] = "N/A";
window.PWA_MODULES["@slyvex/grpc"] = "N/A";
window.PWA_MODULES["@slyvex/core-lib"] = "N/A";

class SlyvexDesktopApp extends FlowApp{
	render(){
		let walletMeta = {"generator":"Slyvex Desktop"}
		let list = [
			['Slyvex','MIT','Copyright (c) 2025 Slyvex Developers'],
		...
		let copyright = [
			['Slyvex Desktop','Copyright (c) 2025 Slyvex Developers', 'All Rights Reserved'],
		...
		return html`
		<flow-caption-bar
			logo="/resources/images/slyvex-logo-light-bg.png">Slyvex Desktop</flow-caption-bar>
		<tab-content for="home">
			...
			<div id='slyvex-resources'>

				<flow-expandable>
					<div slot="title" is="i18n-div" caption>SLYVEX RESOURCES</div>
					<ul style="font-size: 12px;">
						<li><flow-shell-link
							href="https://github.com/slyvex-core/"><flow-i18n>GitHub</flow-i18n></flow-shell-link></li>
						<li><flow-shell-link
							href="https://discord.gg/WTnzChUfgc"><flow-i18n>Discord Chat</flow-i18n></flow-shell-link></li>
						<li><flow-link
			...
			<flow-expandable class="donation-addresses" no-info _icon="fal:donate">
				...
				<p is="i18n-p">
					If you wish to further the development of the slyvex ecosystem, we accept slyvex donations at the following addresses:
				</p>
				${
		...
		<tab-content for="settings">
			<flow-form-control icon="fal:database">
				...
				<p slot="info" is="i18n-p">
					Data Folder location is used for storage by all Slyvex Desktop
					modules. In default configuration this includes Slyvexd blockchain
			...
			<flow-form-control icon="fal:tools">
				...
				<p slot="info" is="i18n-p">
					Advanced mode allows you to manually configure, interact with and monitor Slyvex Desktop services.
				</p>
			</flow-form-control>
			<flow-form-control icon="fal:cog" class="advanced-tool">
				...
				<p slot="info" is="i18n-p">
					When enabled, Slyvex Desktop runs itself and it's services hidden in the background
			...
			<flow-form-control icon="fal:cog" class="advanced-tool">
				...
				<p slot="info" is="i18n-p">When Autocompound enabled, Slyvex Desktop compound UTXOs when there will be compoundable count of unspent transaction outputs (UTXOs).'Use latest Change address' will compund UTXOs using current/latest change address instead of first change address. </p>
			</flow-form-control>
			<!-- flow-form-control icon="fal:drafting-compass" class="advanced-tool">
				...
				<p slot="info" is="i18n-p">
					You can stream Slyvex Desktop metrics to your own StatsD-compatible server.
				</p>
			</flow-form-control-->
			...
			<flow-form-control id="settings-script" icon="fal:tasks">
				...
				<p slot="info" is="i18n-p">
					Configuration editor allows you to customize Slyvex Desktop environment.
					Slyvex Desktop configuration is represented in JSON
			...
			<flow-form-control id="settings-templates" icon="fal:file-alt"
				...
				<p slot="info" is="i18n-p">
					Configuration templates allow you to load pre-made Slyvex Desktop configurations.
				</p>
			</flow-form-control>
			<flow-form-control icon="fal:database">
				...
				<p slot="info" is="i18n-p">
					It will delete datadir (Data Folder) and restart slyvexd node to re-sync
				</p>
			</flow-form-control>
			<flow-form-control icon="fal:database">
				...
				<p slot="info" is="i18n-p">
					Start the slyvexd node without enabling the '--utxoindex' parameter until the synchronization is complete, re-enable this parameter. (Can be used to fix inaccurate balance display)
				</p>
			</flow-form-control>
			<div style="height:192px;"></div>
		</tab-content>
		<tab-content for="wallet" class="wallet" data-active-display="flex">
			<slyvex-wallet .walletMeta='${walletMeta}' hideNetwork hidefaucet 
				_hideQRScanner hideopenwalletlogo></slyvex-wallet>
		</tab-content>
...
//		this.checkForUpdates().then();
	...
	initRPC(){
		let rpc = new FlowRPC({bcastChannel:'slyvex-desktop'});
		this.rpc = rpc;
	...
	async initManager(){
		...
		manager.on("task-start", (daemon)=>{
			...
			this.refreshApps();
			const {wallet} = this;
			if(!wallet || daemon.task.type!='slyvexd' || !this.rpcDisconnect){
				return
			}
		...
		manager.on("before-interrupt", ({daemon, interrupt})=>{
			...
			const {wallet} = this;
			if(!wallet || daemon.task.type!='slyvexd')
				return
		...
		manager.on('sync-status', (data) => {
			//console.log("sync-status:data", data);
			let wallet = this.wallet || this.qS('slyvex-wallet');
	...
	async initWallet() {
		let wallet = this.qS('slyvex-wallet');
		this.wallet = wallet;
		this.applyCompoundConfig();
		let settings = await this.get_default_local_slyvexd_settings();
	...
	async get_default_local_slyvexd_settings() {
		
		let {config:daemons} = await this.get("get-modules-config");
		console.log("############### DAEMONS", daemons);
		let slyvexd = Object.entries(daemons).map(([k,v]) => { 
			const { args } = v;
			const [type, ident] = k.split(':');
			return { type, ident, args};
		}).filter(o=>o.type=='slyvexd').shift();

		if(!slyvexd)
			return null;//{network:"slyvex", port:33455};//{network:"slyvextest", port:33755};
	...
	setTheme(theme){
		...
		if(this.caption)
			this.caption.logo = `/resources/images/slyvex-logo-${theme}-bg.png`
		...
		document.body.dispatchEvent(new CustomEvent("flow-theme-changed"));
		this.querySelector("slyvex-wallet")?.requestUpdate("theme", null)
	}
	initCaption(){
		...
		this.caption.logo = `/resources/images/slyvex-logo-${this.theme||"light"}-bg.png`;
	...
	initTrayMenu() {
		let icon = os.platform=='darwin'?'tray-icon.png':'tray-icon-60.png';
	...
	async initSettings(){
		...
		if(statsdPrefixInput)
			this.statsdPrefix = statsdPrefixInput.value = config.statsdPrefix || "slyvex-desktop.$HOSTNAME";
	...
	initReleaseNotes(){
		...
		dialog.content = 
`#	Welcome to Slyvex Desktop ${pkg.version}!

Useful resources:
- Slyvex Discord: https://discord.gg/Swj4saMdUV
- Slyvex GitHub: https://github.com/slyvex-core/
- Slyvex Desktop GitHub: https://github.com/slyvex-core/slyvex-desktop
...
${changelogContent}`;
	...
	async initDaemons(daemons){
		...
		let {params} = this.getModuleArgs('slyvexd:', daemons);
	...
	initWin(){
		...
		this.closeWin = async(isExit)=>{
			...
			if(isExit !== true && !this.runInBG){
				let {btn} = await FlowDialog.show({
					title:i18n.t("EXIT Slyvex Desktop"),
					body:i18n.t("Are you sure?"),
					btns:[{
						text:i18n.t('Cancel'),
						value:'cancel'
	...
	showApps(daemons) {
		...
		let entries = Object.values(appsMap).map((app) => {
			...
			if(!location && app.engines?.slyvex-desktop) {
			...
			return `${this.debugAppLinks?JSON.stringify(app)+"::"+location:""}
				<flow-window-link
					...
					width="${width}"
					height="${height}"
					icon="resources/images/slyvex-desktop-icon.png"
					resizable
					frame
	...
	async checkForUpdates(force = false) {
		...
		const url = 'https://slyvex.network/slyvex-desktop/version.json';
		...
		if(!data) {
			console.log("missing version data in https://slyvex.network/slyvex-desktop/version.json");
			return false;
		}
		...
		if(is_gt) {
			...
			let {btn} = await FlowDialog.show({
				title:i18n.t("Slyvex Desktop Update"),
			...
			if(btn == 'ok') {
				require('nw.gui').Shell.openExternal('https://slyvex.network');

				let confirm = await FlowDialog.show({
					title:i18n.t("Slyvex Desktop Update"),
					body:html`Please download the latest version from  <flow-link href="https://slyvex.network" target="_blank">https://slyvex.network</flow-link>
					<br/>&nbsp;<br/>
					Slyvex Desktop will now shutdown`,
					btns:[{
						text:i18n.t('Cancel'),
						value:'cancel'
					},{
...
SlyvexDesktopApp.define("slyvex-desktop-app")
