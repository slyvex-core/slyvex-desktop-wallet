...
const {Client:SlyvexGRPC} = require('@slyvex/grpc-node');
const pkg = require("../../package");
const natUpnp = require("@runonflux/nat-upnp");

module.exports = class Slyvexd extends Daemon {
	...
	start() {
		this.log("slyvexd::start() invoked...");
		return new Promise(async (resolve,reject) => {

			if(this.task.conf['reset-peers'])
				this.cleanupResidualData();

			this.log("slyvexd::start() running");
			...
			if (!("uacomment" in args)) {
				const uacomment = `slyvex-desktop_${pkg['version']}`
				this.writeToSink(`SLYVEXD: Adding uacomment=${uacomment} to arguments\n`);
			...
			if (this.task.conf['upnpEnabled'] && args['listen'] && args['listen'].indexOf(":") !== -1) {
				...
				if (externalip) {
					if (!("externalip" in args)) {
						this.writeToSink(`SLYVEXD: Adding externalip=${externalip} to arguments\n`);
					...
					if (publicPort) {
						...
						this.writeToSink(`SLYVEXD: Changing externalip from ${args['externalip']} to ${externalip}:${publicPort} in arguments\n`);
						args['externalip'] = `${externalip}:${publicPort}`;
						if (publicPort !== nodePort) {
							this.writeToSink(`SLYVEXD: Changing listener port from ${nodePort} to ${publicPort} in arguments\n`);
							args['listen'] = `${args['listen'].split(':')[0]}:${publicPort}`;
						}
						if (!("maxinpeers" in args)) {
							this.writeToSink(`SLYVEXD: Adding maxinpeers=${this.upnpMaxInPeers} to arguments\n`);
			...
			const configFile = path.join(this.folder, 'slyvexd.conf');
			...
			await this.VSARGS([this.getDeployableBinary('slyvexd'),'--version']);

			this.run({
				args : async () => {
					let _args_ = [
 						this.getDeployableBinary('slyvexd')
					];
			...
			resolve();
			// mutes all slyvexd processes by default
			this.mute = true;
		})
	}
	...
	async stop() {
		...
		if(this.rpc) {
			...
			try {
				if(!process.env.SLYVEX_JSON_RPC)				
		...
		return new Promise(async (resolve, reject) => {
	        ...
	        try {
				...
				this.data = this.sourcesToData(sources);
	             ...
	             console.log('SLYVEXD monitor:'.bold,ex);
	            return reject(ex);
	        }
