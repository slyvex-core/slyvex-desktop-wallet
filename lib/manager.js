...
class SlyvexProcessManager extends EventEmitter{
	...
	async start(daemons={}) {
		...
		const startupTypeOrder = ['slyvexd','mqtt','mysql','pgsql', 'txgen','app'];
		const typeMap = { };
	...
	async stop() {
		//const shutdownTypeOrder = ['slyvexd','mqtt','mysql','pgsql','simulator','txgen'];
		//const typeMap = { };
	...
	getDeployableBinary(...args) {
		...
		switch(buildType) {
			...
			case 'local': {
				return path.join(os.homedir(),'.slyvex-desktop','bin',utils.platform, ...args)+this.PLATFORM_BINARY_EXTENSION;
			}			
		}
	}
	...
	enumerateApps() {
		...
		return packages.map(({ folder, pkg }) => {
			let { name, description, main, engines, slyvex } = pkg;
			return {
				folder, name, description, main, engines, ...slyvex
			}
		});
	}
...
module.exports = SlyvexProcessManager;
